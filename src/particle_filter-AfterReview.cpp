...
void ParticleFilter::resample() {
   ...
   */
  //cout << "Resample..." << endl;
  // Generate random index to start rasampling
  srand(int(time(nullptr)));
  int index = rand() % num_particles;
 
  vector<double>::iterator iMax = std::max_element(weights.begin(),weights.end());
  double w_max = *iMax;  // max weight 
  double beta = 0.0;     // used for weight wheel 
  
  std::default_random_engine gen(int(time(nullptr)));
  std::uniform_real_distribution<double> dist_real(0,1);

  // Create new particles using weight wheel 
  vector<Particle> new_particles = {};
  double random_num;
  for (int i=0; i<num_particles; ++i){ 
    random_num = dist_real(gen);
    beta += random_num * 2 * w_max;
    
    while (beta > weights[index]) {
        beta -= weights[index];
        index = (index + 1) % num_particles;
    } // End while
    //cout << "Pushback particles[" << index << "]" <<endl;
